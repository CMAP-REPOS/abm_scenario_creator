...
class ABM(object):
    ...
    def __init__(self, abm_dir, sample_rate=0.05, build_db=True):
        ...
        if build_db:
            self._matrices = self._insert_od_data()
    ...
    def _get_boardings(self, node_or_line, split_rail=False):
    ...
    def _get_boarding_groups(self, boardings, num_groups):
        ''' With a dictionary of station/vehicle boardings (generated by
            ABM._get_boardings() as input, return a dictionary with the same
            keys, but where the values are the groups in which the boardings
            fall. These groups will be calculated for equal intervals of the
            natural logs of boardings+1. '''
        # Perform log transformation and calculate min/max values
        ln_boardings = {k: math.log(v+1) for k, v in boardings.iteritems()}
        min_ln_boarding = min((v for v in ln_boardings.itervalues()))
        max_ln_boarding = max((v for v in ln_boardings.itervalues()))

        # Calculate interval endpoints for groups
        interval_size = (max_ln_boarding - min_ln_boarding) / num_groups
        lower_bound = min_ln_boarding
        upper_bounds = {i: min_ln_boarding + (i+1)*interval_size for i in xrange(num_groups)}
        upper_bounds[num_groups-1] = max_ln_boarding  # Ensure last group includes endpoint

        # Assign nodes/lines to groups
        boarding_groups = {}
        for k, v in ln_boardings.iteritems():
    ...
    def _insert_od_data(self):
        ''' Insert travel time and distance matrix data from Emmebank into SQLite DB. '''
        emmebank = _eb.Emmebank(self._emmebank_path)
        matrices = {}
        existing_matrix_tables = set()
        for mode in xrange(1, 7):
            matrices[mode] = {}
            t, d = self._get_matrix_nums(mode)
            for tod in xrange(1, 9):
                matrices[mode][tod] = {
                    't': 'mf{0}{1}'.format(tod, t),
                    'd': 'mf{0}{1}'.format(tod, d)
                }
                for matrix in matrices[mode][tod].itervalues():
                    if matrix not in existing_matrix_tables:
                        self._con.execute('CREATE TABLE {0} (zn_o INTEGER, zn_d INTEGER, value REAL, PRIMARY KEY (zn_o, zn_d))'.format(matrix))
                        matrix_data = emmebank.matrix(matrix).get_data(tod)
                        for zn_o in ABM.zones:
                            for zn_d in ABM.zones:
                                insert_sql = 'INSERT INTO {0} VALUES (?,?,?)'.format(matrix)
                                self._con.execute(insert_sql, (zn_o, zn_d, matrix_data.get(zn_o, zn_d)))
                        self._con.commit()
                        existing_matrix_tables.add(matrix)
                        del matrix_data
    ...
    def _insert_trips(self, trips_csv, is_joint):
        ...
        with open(trips_csv, 'rb') as csvfile:
            ...
            for d in r:
                ...
                matrix_query = 'SELECT value FROM {0} WHERE zn_o={1} AND zn_d={2}'
                if mode <= 6 :  # Private autos
                    time = float(self._con.execute(matrix_query.format(self._matrices[mode][tod]['t'], zn_o, zn_d)).fetchone()[0])
                    distance = float(self._con.execute(matrix_query.format(self._matrices[mode][tod]['d'], zn_o, zn_d)).fetchone()[0])
                elif mode >= 13 :  # Taxis, school buses (assume drive alone, free)
                    time = float(self._con.execute(matrix_query.format(self._matrices[1][tod]['t'], zn_o, zn_d)).fetchone()[0])
                    distance = float(self._con.execute(matrix_query.format(self._matrices[1][tod]['d'], zn_o, zn_d)).fetchone()[0])
                    ...
                    time = float(self._con.execute(matrix_query.format(self._matrices[1][tod]['t'], from_zone, to_zone)).fetchone()[0])
                    distance = float(self._con.execute(matrix_query.format(self._matrices[1][tod]['d'], from_zone, to_zone)).fetchone()[0])
