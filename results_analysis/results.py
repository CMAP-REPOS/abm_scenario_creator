...
class ABM(object):
    ...
    def _get_boarding_groups(self, boardings, num_groups=5, denominator=2):
        ''' With a dictionary of station/vehicle boardings (generated by
            ABM._get_boardings() as input, return a dictionary with the same
            ...
            denominator (e.g. group 1: first third of boardings; group 2:
            first third of boardings not in group 1; etc.). '''
        # Order boardings
        ordered_boardings = sorted(boardings.itervalues())
        num_boardings = len(ordered_boardings)

        # Calculate interval endpoints for groups
        def find_group_endpoints(num_boardings, num_groups, denominator, group=1, endpoints=None):
            if not endpoints:
                endpoints = {}
            ...
            if group == num_groups:
                ...
                end_pos = int(round(((num_boardings - start_pos) / float(denominator)) + start_pos))
            endpoints[group] = end_pos
            if group < num_groups and end_pos < num_boardings:
                return find_group_endpoints(num_boardings, num_groups, denominator, group+1, endpoints)
            else:
                return endpoints

        endpoints = find_group_endpoints(num_boardings, num_groups, denominator)
